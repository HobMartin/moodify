...
export const getCurrentUser = async (session: AuthSession) => {
...
export const createUserPlaylist = async ({
  ...
  return customPost(
    ...
    {
      name,
      description: "Auto generated playlist by Moodify",
    }
  );
};
...
export const availableGenre = async (session: AuthSession) => {
  return customGet(
...
export const getRecommendationsByMood = async ({
  ...
  let endpoint = new URL(`https://api.spotify.com/v1/recommendations?limit=30`);

  Object.entries(query).forEach(([key, value]) => {
    endpoint.searchParams.append(key, decodeURI(value.toString()));
  });
  console.log(endpoint.toString());

  const data = await customGet(endpoint.toString(), session);
  return data;
};
